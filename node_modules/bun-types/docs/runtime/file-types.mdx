...
Bun uses the file extension to pick the built-in _loader_ that parses the file. Every loader has a name, such as `js`, `tsx`, or `json`. These names are used when building [plugins](/docs/bundler/plugins) that extend Bun with custom loaders.
...
CSS files can be directly imported. This is primarily useful when [bundling HTML](/docs/bundler/html-static#importing-css-in-javascript), where CSS is bundled alongside HTML.
